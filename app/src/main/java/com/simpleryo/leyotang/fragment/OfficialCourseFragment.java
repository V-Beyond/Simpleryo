...
import com.simpleryo.leyotang.bean.CommonBean;
...
public class OfficialCourseFragment extends XLibraryLazyFragment {
    List<CommonBean> excellentListBeans = new ArrayList<>();
                ...
                if (courseListBean.getCode().equalsIgnoreCase("0")){
                    if (courseListBean.getData()!=null&&courseListBean.getData().size()>0){
                        ...
                        for (CourseListBean.DataBeanX dataBean:hotCourseList){
                            multipleItem=new MultipleItem(MultipleItem.HOMEINTRODUCTORYCOURSE);
