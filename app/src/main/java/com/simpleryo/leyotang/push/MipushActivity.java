...
//                sysMessage 系统消息
            if (extrs.get("type").equalsIgnoreCase("courseClass")) {
                Intent courseClass = new Intent(MipushActivity.this, MainActivity.class);
                courseClass.putExtra("type", "push");
                courseClass.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                startActivity(courseClass);
                finish();
            } else if (extrs.get("type").equalsIgnoreCase("payComplete")) {
                Intent payComplete = new Intent(MipushActivity.this, MainActivity.class);
                payComplete.putExtra("type", "push");
                payComplete.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                startActivity(payComplete);
                finish();
            } else if (extrs.get("type").equalsIgnoreCase("sysMessage")) {
                Intent sysMessage = new Intent(MipushActivity.this, MyMsgActivity.class);
